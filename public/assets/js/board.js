...
function handleContributorSave(event) {
  ...
  if (contributor) {
    displayMessage(`${contributorEmail} already has access to this board`, 'danger');
    return;
  }
  ...
  .then(function() {
    init();
    displayMessage(`Successfully added ${contributorEmail} to this board!`, 'success');
  })
  .catch(function() {
    displayMessage(`Contributor must have a user account to collaborate. Cannot find user with email: ${contributorEmail}`, 'danger');
  });
}
